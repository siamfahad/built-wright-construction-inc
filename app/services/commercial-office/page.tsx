...
import { 
  ...
  ChevronDown, ArrowRight, ShieldCheck, Building2, Store, HardHat, Warehouse, Construction
} from "lucide-react";
...
const subCategories = [
  {
    ...
    icon: Building2,
    description: "Creating high-productivity corporate environments. We handle everything from glass partition installations to integrated data cabling and ergonomic workspace design."
  },
  {
    id: "ii",
    title: "Retail & Restaurant Construction",
    icon: Store,
    description: "Specialized commercial construction for the hospitality and retail sectors. We focus on brand-aligned aesthetics, durable finishes, and strict health code compliance."
  },
  {
    id: "iii",
    title: "Tenant Improvements",
    icon: HardHat,
    description: "Efficiently modifying existing commercial spaces to suit new occupants. We minimize downtime while maximizing the functional layout of your leased property."
  },
  {
    id: "iv",
    title: "Warehouse Interior Construction",
    icon: Warehouse,
    description: "Industrial-grade interior solutions including mezzanine installations, heavy-duty shelving, and specialized flooring for logistics and manufacturing hubs."
  },
  {
    id: "v",
    title: "Commercial Renovations",
    icon: Construction,
    description: "Complete overhaul of dated commercial assets. Upgrading structural integrity and aesthetic appeal to increase property value and attract premium tenants."
  }
];
    ...
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-sans relative overflow-x-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 z-[110] bg-[var(--background)]/90 backdrop-blur-xl border-b border-foreground/5 px-6 py-4 md:py-6">
        <div className="mx-auto max-w-7xl flex justify-between items-center">
            ...
            <button onClick={() => setIsNavOpen(!isNavOpen)} className="flex flex-col items-start group text-left">
               <div className="flex items-center gap-3">
                 ...
                 <div className="flex items-center gap-2">
                   <span className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-600">Commercial Fit-Outs</span>
                   <ChevronDown size={14} className={`text-blue-600 transition-transform duration-300 ${isNavOpen ? 'rotate-180' : ''}`} />
                 </div>
               </div>
                  ...
                  <div className="p-3 grid grid-cols-1 gap-1 max-h-[60vh] overflow-y-auto">
                    {navigationDivisions.map((nav) => (
                      <Link 
                        ...
                        className={`flex items-center gap-4 p-3 rounded-xl transition-all ${nav.slug === 'commercial-office' ? 'bg-blue-600 text-white' : 'hover:bg-foreground/5'}`}
                        onClick={() => setIsNavOpen(false)}
                      >
                        <nav.icon size={16} className={nav.slug === 'commercial-office' ? 'text-white' : 'text-blue-600'} />
          ...
          <Link href="/services" className="flex items-center gap-2 bg-foreground/5 hover:bg-blue-600 hover:text-white p-2 md:px-4 md:py-2 rounded-full transition-all">
            <X size={20} />
          </Link>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-7xl">
          <motion.div initial={{ opacity: 1, y: 0 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-5xl md:text-[8rem] font-black tracking-tighter uppercase italic leading-[0.8] mb-8">
              Commercial <br /> <span className="text-blue-600 font-outline-2">Fit-Outs</span>
            </h1>
            <p className="text-lg md:text-xl max-w-2xl opacity-70 leading-relaxed font-medium border-l-4 border-blue-600 pl-6">
              Engineering high-performance commercial spaces. Built Wright delivers professional-grade construction for the modern business landscape.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-12 px-6 pb-40 min-h-screen">
        <div className="mx-auto max-w-7xl">
                ...
                >
                  <div className="relative z-10 grid md:grid-cols-12 gap-8 items-start">
                    <div className="md:col-span-1 flex flex-col items-center gap-4">
                      <span className="text-blue-600 font-black italic text-xl">0{index + 1}</span>
                    ...
                    <div className="md:col-span-7 space-y-4">
                      <h3 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter text-blue-600">
                        {sub.title}
                      </h3>
                    ...
                    <div className="md:col-span-4 flex md:justify-end md:items-center h-full">
                      <div className="flex items-center gap-4 px-6 py-3 rounded-full border border-foreground/10 bg-background/50">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">Turnkey Solution</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
          ...
          <div className="mt-20 flex justify-center border-t border-foreground/5 pt-12">
            <Link href="/contact" className="bg-blue-600 text-white px-10 py-6 rounded-full font-black uppercase text-[12px] tracking-[0.2em] shadow-2xl">
              Start Your Build <ArrowRight className="inline ml-2" />
            </Link>
          </div>
        </div>
      </section>
