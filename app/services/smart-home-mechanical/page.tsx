...
import { 
  ...
  ChevronDown, ArrowRight, ShieldCheck, Cpu, Droplets, Thermometer, Lightbulb, TabletSmartphone
} from "lucide-react";
...
const subCategories = [
  {
    id: "i",
    title: "Electrical Installations & Panel Upgrades",
    icon: Zap,
    description: "High-capacity electrical engineering for modern estates. We handle everything from main panel upgrades to specialized wiring for high-demand appliances and EV charging stations."
  },
  {
    id: "ii",
    title: "Plumbing Systems",
    icon: Droplets,
    description: "Precision-engineered potable water and drainage systems. We specialize in luxury fixture integration, tankless water heaters, and advanced filtration technologies."
  },
  {
    id: "iii",
    title: "HVAC Installation",
    icon: Thermometer,
    description: "Next-generation climate control. We design and install high-efficiency forced air, radiant floor heating, and multi-zone cooling systems for optimal thermal comfort."
  },
  {
    id: "iv",
    title: "Smart Home Automation",
    icon: TabletSmartphone,
    description: "Seamless control of your environment. Integrated systems for security, climate, and entertainment, accessible from anywhere in the world via centralized interfaces."
  },
  {
    id: "v",
    title: "Custom Lighting Design",
    icon: Lightbulb,
    description: "Architectural lighting solutions that define space and mood. We implement smart dimming, automated scenes, and low-voltage aesthetic lighting for interiors and landscapes."
  }
];
    ...
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-sans relative overflow-x-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 z-[110] bg-[var(--background)]/90 backdrop-blur-xl border-b border-foreground/5 px-6 py-4 md:py-6">
        <div className="mx-auto max-w-7xl flex justify-between items-center">
            ...
            <button onClick={() => setIsNavOpen(!isNavOpen)} className="flex flex-col items-start group text-left">
               <div className="flex items-center gap-3">
                 ...
                 <div className="flex items-center gap-2">
                   <span className="text-[10px] font-black uppercase tracking-[0.4em] text-cyan-500">Smart Infrastructure</span>
                   <ChevronDown size={14} className={`text-cyan-500 transition-transform duration-300 ${isNavOpen ? 'rotate-180' : ''}`} />
                 </div>
               </div>
                  ...
                  <div className="p-3 grid grid-cols-1 gap-1 max-h-[60vh] overflow-y-auto">
                    {navigationDivisions.map((nav) => (
                      <Link 
                        ...
                        className={`flex items-center gap-4 p-3 rounded-xl transition-all ${nav.slug === 'smart-home-mechanical' ? 'bg-cyan-500 text-white' : 'hover:bg-foreground/5'}`}
                        onClick={() => setIsNavOpen(false)}
                      >
                        <nav.icon size={16} className={nav.slug === 'smart-home-mechanical' ? 'text-white' : 'text-cyan-500'} />
          ...
          <Link href="/services" className="flex items-center gap-2 bg-foreground/5 hover:bg-cyan-500 hover:text-white p-2 md:px-4 md:py-2 rounded-full transition-all">
            <X size={20} />
          </Link>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-7xl">
          <motion.div initial={{ opacity: 1, y: 0 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-5xl md:text-[8rem] font-black tracking-tighter uppercase italic leading-[0.8] mb-8">
              Smart <br /> <span className="text-cyan-500 font-outline-2">Systems</span>
            </h1>
            <p className="text-lg md:text-xl max-w-2xl opacity-70 leading-relaxed font-medium border-l-4 border-cyan-500 pl-6">
              The nervous system of the modern home. We integrate high-performance mechanical systems with seamless automation for an intelligent living experience.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-12 px-6 pb-40 min-h-screen">
        <div className="mx-auto max-w-7xl">
                ...
                >
                  <div className="relative z-10 grid md:grid-cols-12 gap-8 items-start">
                    <div className="md:col-span-1 flex flex-col items-center gap-4">
                      <span className="text-cyan-500 font-black italic text-xl">0{index + 1}</span>
                      <div className="p-4 rounded-2xl bg-cyan-500 text-white shadow-xl">
                        <sub.icon size={24} />
                      </div>
                    </div>

                    <div className="md:col-span-7 space-y-4">
                      <h3 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter text-cyan-500">
                        {sub.title}
                      </h3>
          ...
          <div className="mt-20 flex justify-center border-t border-foreground/5 pt-12">
            <Link href="/contact" className="bg-cyan-500 text-white px-10 py-6 rounded-full font-black uppercase text-[12px] tracking-[0.2em] shadow-2xl">
              Automate Your Home <ArrowRight className="inline ml-2" />
            </Link>
          </div>
        </div>
      </section>
